...
export default {
    ...
    data() {
        return {
            ...
            days_anticipation_item_exp_date: 0,
            price_options: [
                {
                    id: 1,
    ...
    async created() {
        ...
        this.events();
        //console.log(this.configuration2);
    ...
    methods: {
        ...
        dateDiff(fecha1, fecha2) {
            ...
            return Math.round(diferenciaDias); // Redondear por si hay diferencias de tiempo menores
        },
        async clickAddItem(item, index, input = false) {
            this.loading = true;

            //Verificar si producto esta por vencer o esta vencido
            let hoy = this.getFechaActual();
            let days = this.configuration2.days_anticipation_item_exp_date;
            let date_of_due = item.date_of_due;
            console.log(hoy);
            console.log(days)
            console.log(date_of_due);


            //Verificar si el producto esta por vencer o esta vencido

            if(date_of_due <= hoy){
                this.$notify({
                        title: "",
                        message: "¡¡Producto está vencido!!, fecha de vencimiento : " + date_of_due,
                        type: "error",
                        duration: 1400
                    });
                    this.loading = false;
                    this.initFocus();
                 return;
                //return this.$message.error("" + date_of_due);
            }else{
                 //por vencer
                let diff = this.dateDiff(hoy,date_of_due)
                console.log(diff);

                if(diff <= days){
                    this.$notify({
                        title: "",
                        message: "¡¡Producto está por vencer!!, fecha de vencimiento : " + date_of_due,
                        type: "warning",
                        duration: 1400
                    });
                    //return this.$message.warning("Producto está por vencer, fecha de vencimiento : " + date_of_due);
                }
